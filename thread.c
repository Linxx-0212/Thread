...
grn_thread *grn_new_thread(bool alloc_stack) {
  UNUSED(alloc_stack);
  grn_thread *new_thread = (grn_thread*)malloc(sizeof(grn_thread));
  new_thread->id = atomic_next_id();
  new_thread->status = WAITING;
  if (alloc_stack) {
    new_thread->stack = (uint8_t*)malloc(STACK_SIZE);
...
void debug_thread_print(grn_thread *thread) {
  ...
  fprintf(stderr, ":: rbp reg:\t 0x%08" PRIu64 "x\n", thread->context.rbp);
  fprintf(stderr, ":: rbx reg:\t 0x%08" PRIu64 "x\n", thread->context.rbx);
  fprintf(stderr, ":: r12 reg:\t 0x%08" PRIu64 "x\n", thread->context.r12);
  fprintf(stderr, ":: r13 reg:\t 0x%08" PRIu64 "x\n", thread->context.r13);
  fprintf(stderr, ":: r14 reg:\t 0x%08" PRIu64 "x\n", thread->context.r14);
  fprintf(stderr, ":: r15 reg:\t 0x%08" PRIu64 "x\n", thread->context.r15);
  fflush(stderr);
}
